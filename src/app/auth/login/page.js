  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.error) {
        ...
        if (response.error && Array.isArray(response.error)) {
          ...
          response.error.forEach((fieldError) => {
            if (
              fieldError.property === "email" ||
              fieldError.property === "password"
            ) {
      ...
      if (response.user.role.scope === "global") {
        ...
        router.push(redirectAfterLogin?.includes("/admin") ? redirectAfterLogin : "/admin/dashboard");
      } else if (response.user.role.scope === "agency") {
        const redirectAfterLogin =
          sessionStorage.getItem("redirectAfterLogin");
        sessionStorage.removeItem("redirectAfterLogin");

        router.push(redirectAfterLogin?.includes("/agency") ? redirectAfterLogin : `/agency/dashboard/${response.user.role.agency.id}`);
      }
    } catch (err) {
        ...
        <Card className="w-full max-w-md shadow-2xl bg-white rounded-xl border-0">
          ...
          <CardHeader className="pt-8 pb-4">
            <div className="flex justify-center mb-6">
              <div
                onClick={() => router.push("/")}
                className="relative cursor-pointer w-12 h-12 bg-teal-100 rounded-full flex items-center justify-center"
              >
                <Image
          ...
          <CardFooter className="pt-2 pb-8 flex flex-col items-center">
            <p className="text-sm text-gray-600 mt-6">
              ...
              <Link
                href={`https://wa.me/57${
                  landingAssets.contact
                }?text=${encodeURIComponent(
                  "Saludos! me gustaría crear una cuenta con ustedes"
                )}`}
                target="_blank"
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-12 text-white">
          ...
          <div className="space-y-4">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="font-semibold text-xl">
                  {t("login.sidebar.feature1.title")}
                </h3>
                <p className="text-teal-100">
                  {t("login.sidebar.feature1.description")}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="font-semibold text-xl">
                  {t("login.sidebar.feature2.title")}
                </h3>
                <p className="text-teal-100">
                  {t("login.sidebar.feature2.description")}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="font-semibold text-xl">
                  {t("login.sidebar.feature3.title")}
                </h3>
                <p className="text-teal-100">
                  {t("login.sidebar.feature3.description")}
                </p>
              </div>
            </div>
          </div>
